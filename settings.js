...
function cdnRel(p){p=String(p||'').trim();if(!p)return'';if(/^(https?:)?\/\//i.test(p)||/^(data|blob):/i.test(p))return vurl(p);return vurl(String(global.CDNurl||global.CDNURL||global.CCH_CDN_PREFIX||global.SQL_CDN_PREFIX||CDN_PREFIX||'').replace(/\/?$/,'/')+p.replace(/^\/+/,''));}
...
function fileCandidates(p){p=String(p||'').trim();if(!p)return[];if(/^(https?:)?\/\//i.test(p)||/^(data|blob):/i.test(p))return[vurl(p)];var locals=[localRel(p)],remotes=mappedCdnValues(p).map(cdnRel);remotes.push(cdnRel(p));var b=baseName(p);if(b&&b!==p)remotes.push(cdnRel(b));var arr=preferCdnFiles()?remotes.concat(locals):locals.concat(remotes);return uniqueUrls(arr)}

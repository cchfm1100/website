...
(function(){
  ...
  function _cdnBases(){const out=[];const add=v=>{v=String(v||'').trim().replace(/\/+$/,'');if(v&&!out.includes(v))out.push(v)};add(window.CCH_CDN_PREFIX||window.SQL_CDN_PREFIX||window.CDNurl||window.CDNURL||'');try{(window.CCH_CDN_FALLBACKS||[]).forEach(add)}catch(e){}add('https://cdn.jsdelivr.net/gh/cchfm1100/website@main');return out;}

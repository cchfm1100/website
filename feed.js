...
(function(){
  ...
  function cchCdnUrl(file){const base=window.CCH_CDN_PREFIX||window.SQL_CDN_PREFIX||(String(window.CDNurl||window.CDNURL||'').replace(/\/+$/,'')+'/');return String(base||'').replace(/\/?$/, '/')+String(file||'').replace(/^\/+/, '')};window.cchCdnUrl=window.cchCdnUrl||cchCdnUrl
  function settingUrlCandidates(file){const want=sqliteName(file);let remote='';try{const root=(typeof settings!=='undefined'&&settings&&settings[0])?settings[0]:null;if(root&&Array.isArray(root.feeds)){for(const group of root.feeds){const scripts=group&&group.script;if(!Array.isArray(scripts)) continue;for(const m of scripts){if(!m||typeof m!=='object') continue;for(const k of Object.keys(m)){if(sqliteName(k)===want){remote=String(m[k]||'').trim().replace(/\.js(\?|#|$)/i,'.sqlite$1');break}}if(remote) break}if(remote) break}}}catch(e){}if(!remote) remote=cchCdnUrl(want);const local=want;const order=cchSqlitePreferCdn()?[remote,local]:[local,remote];const out=[];for(const raw of order){if(!raw) continue;let u=String(raw).trim();try{u=new URL(versionedUrl(u),location.href).href}catch(e){try{u=versionedUrl(u)}catch(e2){}}if(u&&!out.includes(u)) out.push(u)}return out.length?out:[new URL(want,location.href).href]}
  ...
  async function getWorker(file){const f=sqliteName(file);if(window.__SQLITE_FEED_WORKERS__[f]) return window.__SQLITE_FEED_WORKERS__[f];const mod=await loadHttpVfsModule();const urls=settingUrlCandidates(f);let last=null;for(const url of urls){try{const worker=await mod.createDbWorker([{from:'inline',config:{serverMode:'full',requestChunkSize:4096,url:url,cacheBust:String(window.version||'')}}],getWorkerUrl(),versionedUrl(SQLITE_WASM_CDN),Infinity);try{await queryRows(worker,"SELECT name FROM sqlite_master LIMIT 1")}catch(probeErr){throw probeErr}window.__SQLITE_FEED_WORKERS__[f]=worker;window.__SQLITE_FEED_WORKER_URLS__[f]=url;return worker}catch(e){last=e}}throw last||new Error('SQLite database load failed: '+f)}
  async function queryRows(worker,sql){if(worker&&worker.db&&typeof worker.db.query==='function'){try{return rowsFromExec(await worker.db.query(sql))}catch(e){}}if(worker&&worker.db&&typeof worker.db.exec==='function') return rowsFromExec(await worker.db.exec(sql));return []}
